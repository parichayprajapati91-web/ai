...
async function setupVite(server, app2) {
  const vite = await (0, import_vite.createServer)({
    configFile: (0, import_path2.resolve)(__dirname, "../vite.config.mjs"),
    server: {
      middlewareMode: true,
...
var import_vite, import_url, import_path2, import_fs2, import_path3, import_meta, __dirname, viteLogger;
var init_vite = __esm({
  "server/vite.ts"() {
    "use strict";
    ...
    init_nanoid();
    import_meta = {};
    __dirname = (0, import_path2.dirname)((0, import_url.fileURLToPath)(import_meta.url));
    viteLogger = (0, import_vite.createLogger)();
  }
});
...
__export(schema_exports, {
  ...
  voiceDetectionRequestSchema: () => voiceDetectionRequestSchema,
  voiceDetectionResponseSchema: () => voiceDetectionResponseSchema
});
...
// shared/schema.ts
var apiKeys = (0, import_sqlite_core.sqliteTable)("api_keys", {
  id: (0, import_sqlite_core.integer)("id", { mode: "number" }).primaryKey({ autoIncrement: true }),
  ...
  createdAt: (0, import_sqlite_core.text)("created_at").default(import_drizzle_orm2.sql`(CURRENT_TIMESTAMP)`).notNull()
});
var requestLogs = (0, import_sqlite_core.sqliteTable)("request_logs", {
  id: (0, import_sqlite_core.integer)("id", { mode: "number" }).primaryKey({ autoIncrement: true }),
  apiKeyId: (0, import_sqlite_core.integer)("api_key_id", { mode: "number" }).references(() => apiKeys.id),
  language: (0, import_sqlite_core.text)("language").notNull(),
  classification: (0, import_sqlite_core.text)("classification").notNull(),
  // AI_GENERATED or HUMAN
  confidenceScore: (0, import_sqlite_core.real)("confidence_score").notNull(),
  explanation: (0, import_sqlite_core.text)("explanation"),
  timestamp: (0, import_sqlite_core.text)("timestamp").default(import_drizzle_orm2.sql`(CURRENT_TIMESTAMP)`).notNull(),
  clientIp: (0, import_sqlite_core.text)("client_ip")
});
var insertApiKeySchema = createInsertSchema(apiKeys).omit({ id: true, createdAt: true });
var insertRequestLogSchema = createInsertSchema(requestLogs).omit({ id: true, timestamp: true });
var voiceDetectionRequestSchema = external_exports.object({
  language: external_exports.enum(["Tamil", "English", "Hindi", "Malayalam", "Telugu"]),
  audioFormat: external_exports.literal("mp3"),
  audioBase64: external_exports.string().min(1, "Audio data is required")
});
var voiceDetectionResponseSchema = external_exports.object({
  status: external_exports.enum(["success", "error"]),
  language: external_exports.string(),
  classification: external_exports.enum(["AI_GENERATED", "HUMAN"]),
  confidenceScore: external_exports.number().min(0).max(1),
  explanation: external_exports.string()
});

// server/db.ts
var sqlite = new import_better_sqlite32.default(process.env.DATABASE_URL || "./sqlite.db");
...
var DatabaseStorage = class {
  async getApiKey(key) {
    const [apiKey] = await db.select().from(apiKeys).where((0, import_drizzle_orm3.eq)(apiKeys.key, key));
    return apiKey;
  }
  async createApiKey(owner) {
    const key = "sk_" + (0, import_crypto.randomBytes)(16).toString("hex");
  ...
  async logRequest(log2) {
    const [entry] = await db.insert(requestLogs).values(log2).returning();
    return entry;
  }
  async getRecentLogs(limit = 50) {
    return db.select().from(requestLogs).orderBy((0, import_drizzle_orm3.desc)(requestLogs.timestamp)).limit(limit);
  }
  async deleteLogs() {
    await db.delete(requestLogs);
  }
  async getStats() {
    const [total] = await db.select({ count: (0, import_drizzle_orm3.count)() }).from(requestLogs);
    const [ai] = await db.select({ count: (0, import_drizzle_orm3.count)() }).from(requestLogs).where((0, import_drizzle_orm3.eq)(requestLogs.classification, "AI_GENERATED"));
    const [human] = await db.select({ count: (0, import_drizzle_orm3.count)() }).from(requestLogs).where((0, import_drizzle_orm3.eq)(requestLogs.classification, "HUMAN"));
    return {
...
var api = {
  voiceDetection: {
    detect: {
      ...
      responses: {
        ...
        400: external_exports.object({ status: external_exports.literal("error"), message: external_exports.string() }),
        401: external_exports.object({ status: external_exports.literal("error"), message: external_exports.string() })
      }
    },
    forensics: {
      method: "POST",
      ...
      input: external_exports.object({
        ...
        gender: external_exports.enum(["male", "female", "unknown"]).optional()
      }),
      responses: {
        200: external_exports.object({
          status: external_exports.literal("success"),
          classification: external_exports.enum(["AI", "HUMAN", "UNCERTAIN"]),
          human_probability: external_exports.number(),
          ai_probability: external_exports.number(),
          confidence: external_exports.number(),
          forensic_analysis: external_exports.record(external_exports.any())
        }),
        400: external_exports.object({ status: external_exports.literal("error"), message: external_exports.string() }),
        401: external_exports.object({ status: external_exports.literal("error"), message: external_exports.string() })
      }
  ...
  admin: {
    generateKey: {
      ...
      input: external_exports.object({ owner: external_exports.string() }),
      responses: {
    ...
    getStats: {
      ...
      responses: {
        200: external_exports.object({
          ...
          aiDetected: external_exports.number(),
          humanDetected: external_exports.number(),
          recentLogs: external_exports.array(external_exports.custom())
        })
      }
    },
    deleteLogs: {
      ...
      input: external_exports.object({ logIds: external_exports.array(external_exports.number()) }),
      responses: {
        200: external_exports.object({ status: external_exports.literal("success"), message: external_exports.string() }),
        400: external_exports.object({ status: external_exports.literal("error"), message: external_exports.string() })
      }
    }
  }
...
function improvedFallbackDetection(buffer, language) {
  ...
  const classification = aiProbability > 0.5 ? "AI_GENERATED" : "HUMAN";
  const sizeConfidence = Math.abs(aiProbability - 0.5) * 2;
...
async function registerRoutes(httpServer2, app2) {
  ...
  app2.post(api.voiceDetection.detect.path, validateApiKey, async (req, res) => {
    try {
      ...
      try {
        ...
        try {
          ...
          const output = (0, import_child_process.execSync)(`"${pythonExe}" "${pythonScriptPath}"`, {
            input: pythonInput,
            encoding: "utf-8",
            cwd: process.cwd(),
            timeout: 3e4,
        ...
        const classification = forensicResult.classification === "HUMAN" ? "HUMAN" : forensicResult.classification === "AI" ? "AI_GENERATED" : "HUMAN";
        const confidence = forensicResult.confidence / 100;
        const explanation = forensicResult.classification === "AI" ? `AI-generated voice detected in ${input.language}: artificial patterns and synthetic characteristics identified.` : `Human voice detected in ${input.language}: natural acoustic patterns and organic variations confirmed.`;
  ...
  app2.post(api.voiceDetection.forensics.path, validateApiKey, async (req, res) => {
    try {
      ...
      try {
        ...
        const forensicResult = result.classification;
        const classification = forensicResult.classification === "HUMAN" ? "HUMAN" : forensicResult.classification === "AI" ? "AI" : "UNCERTAIN";
        await storage.logRequest({
          apiKeyId: req.apiKey.id,
          language: input.language || "unknown",
  ...
  app2.post(api.admin.generateKey.path, async (req, res) => {
    try {
  ...
  app2.get(api.admin.getStats.path, async (req, res) => {
    try {
  ...
  app2.delete(api.admin.deleteLogs.path, async (req, res) => {
    try {
...
function serveStatic(app2) {
  ...
  console.log(`[serveStatic] Path exists: ${import_fs.default.existsSync(distPath)}`);
